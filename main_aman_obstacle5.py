...
def callback(data):
    global avoidance, obstacle_number, oldtime, status_ros, start_ros_time, steering_angle, speed, lane_left, track_detected, start_time, display_time, fc, FPS
    if status_ros == False:
    ...
    def perspectiveWarp(inpImage):
        global avoidance, obstacle_number, oldtime, steering_angle, lane_left, speed, deviation
        speed = 0
        ...
        if current_time >= oldtime + 1.5 and oldtime != 0:
            avoidance = 2
        cv2.putText(
            img,
    ...
    if int(FPS) <= 20:
        ...
        cv2.putText(
            img, "Please Upgrade your PC", (220, 340), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 4, cv2.LINE_AA
        )
